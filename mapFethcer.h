...
char* returnMapValue(float percentage, char writeValue[5])
{
	writeValue[2] = '.';
	if(percentage < 0)
		percentage = 0;
...
//Creates dummy map. Only used in testers.
//TO-DO Create somewhat usable maps
void createOuterWheelMap()
{
	float v[10][10];
	float k = 0.00;
	for(int i = 0; i < 10; i++)
...
//Creates dummy map. Onlly used in testers.
//TO-DO Create somewhat usable maps
void createInnerWheelMap()
{
	float v[10][10];
	float k = 0.00;
	for(int i = 0; i < 10; i++)
